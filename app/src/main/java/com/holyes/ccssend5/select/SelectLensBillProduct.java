...
//        listview.setOnItemClickListener(new listViewClick());
        ...
        if (lsv_aim != null && !lsv_aim.isEmpty()) {
            if ("P_Dv_OutStock_Lens_Z_D_Bill_BeInStock".equals(lsv_aim) ||
...
//                if (lsv_aim.equals("P_Dv_OutStock_Lens_Z_L_Bill_BeInStock")) {
                    dataList = accWeb.GetDowLoadLensInvoiceDetail(orderno);
//                }
                if (dataList.size() == 0) {
...
//        Collections.sort(list, new SortListMapComparator("goodsid"));
        SimpleAdapter adapter = new SimpleAdapter(SelectLensBillProduct.this, list,
                R.layout.list_scan_lens_detail, new String[]{"Product_id", "RefractiveIndex", "Diopter","Astigmatism", "NoSendGoodQty"},
                new int[]{R.id.txt_list1, R.id.txt_refractive, R.id.txt_list2, R.id.txt_list3, R.id.txt_list4});
